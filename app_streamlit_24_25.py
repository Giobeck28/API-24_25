...
vacs =[6,7,15, 22, 23, 30,31]
file_name = '/Users/gio/Downloads/stats_2024_25.xlsm'
nombre_manche_Y = 36
nombre_manche_Q = 12
#nombre_manche_V = 5
...
presences = 107 - (df_points == 0).sum(axis=0)
...
if page == pages[1]:
    ...
    for j in range(nombre_manche_Y):
        ...
        if j in range(nombre_manche_Q*2,nombre_manche_Y):        
            ...
            if j == nombre_manche_Y -1 :
                ...
                # Temp Patch for stats before season end
                ax.plot((nombre_manche_Q*2 + 1,nombre_manche_Y) , (presences[nombre_manche_Q*2:nombre_manche_Y-1].mean(),presences[nombre_manche_Q*2:nombre_manche_Y-1].mean()), linewidth= 3, color = '#2E3033', linestyle='-')
                ax.plot((0,40) , (presences[0:nombre_manche_Y-1].mean(),presences[0:nombre_manche_Y-1].mean()), linewidth= 5, color = 'y', linestyle=':')
    ...
    # Temp Patch for stats before season end
    m_T3 = str(round(presences[nombre_manche_Q*2:nombre_manche_Y-1].mean()))
    m_Y = str(round(presences[0:nombre_manche_Y-1].mean()))
...
if page == pages[4]:
  ...
  # title (ou of chart) 
    original_title = f'<p style="font-family:Courier; color:Black; font-size: 50px;">Moyennes vs. Présences </p>'
    #st.markdown(original_title, unsafe_allow_html=True)
